...
Old_Ghost::Old_Ghost(float tr, float tg, float tb, int tx, int ty, float tz) : Creature(tx,ty,tz)
{
...
void Old_Ghost::onTileChange()
{
	//std::cout << "Ghost tile: " << tileX << ", " << tileY << std::endl;	
}

bool Old_Ghost::WallCheck(int agle)
{
	if (agle == 0)
...
int Old_Ghost::TargetPythagoras(int agle)
{
	int tx = 0;
	int ty = 0;
